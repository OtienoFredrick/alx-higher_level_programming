...
def matrix_mul(m_a, m_b):
    ...
    # Check if all elements in m_a and m_b are int or float
    ...
    # Check if each row of m_a is the same size
    if not all(len(row) == len(m_a[0]) for row in m_a):
        raise TypeError("each row of m_a must be of the same size")

    # Check if each row of m_b is the same size
    ...
    for row in m_a:
        ...
        for i in range(len(m_b[0])):
            product = sum(row[j] * m_b[j][i] for j in range(len(m_b)))
